...
int main(int argc, char **argv) {
  ...
  while (getline(
      data, line)) { // loops through the txt file until there are no lines left
    ...
    for (int cnt = 0; cnt < 3; cnt++) {
      ...
      words[cnt] = line.substr(
          0, pos); // separating the words and storing them in the array
      line.erase(0, pos + 1); // deleting the current words from the string so
                              // we can continue
    }

    words[2].pop_back(); // removing line ending
    characters.push_back(
        character{stoi(words[0]), words[1], words[2]}); // create the object for
  ...
  while (getline(
      matchups,
      line)) { // loops through the txt file until there are no lines left
    ...
    auto A_idx_pos =
        find_if(characters.begin(), characters.end(),
                [A_id](const auto &obj) -> bool { return obj.id == A_id; });
    A_idx = distance(characters.begin(), A_idx_pos);

    auto B_idx_pos =
        find_if(characters.begin(), characters.end(),
                [B_id](const auto &obj) -> bool { return obj.id == B_id; });
    ...
    if (A_votes > B_votes) {
      characters[A_idx].rating =
          A_cur_rating + K * B_wc * A_votes / (A_votes + B_votes);
      characters[B_idx].rating =
          B_cur_rating - K * B_wc * A_votes / (A_votes + B_votes);
      characters[A_idx].wins++;
    } else if (B_votes > A_votes) {
      characters[B_idx].rating =
          B_cur_rating + K * A_wc * B_votes / (B_votes + A_votes);
      characters[A_idx].rating =
          A_cur_rating - K * A_wc * B_votes / (B_votes + A_votes);
  ...
  for (auto &obj : characters) {
    obj.geomean =
        pow(obj.win_rate * obj.attendance * obj.balance * obj.rating, 1.0 / 4);
  }

  // This section sorts the characters vector from the highest geomean to the
  // lowest
  ...
  int rank = 1;
  file_out << "Rank,ID,Name,Anime,Matches,Wins,Votes+,Votes-,WinRate,"
              "Attendance,Balance,Rating,Geomean"
           << endl;
